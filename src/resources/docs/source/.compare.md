...
curl -X POST \
    ...
    -H "Accept: application/json" \
    -H "Authorization: Bearer {token}" \
    -d '{"fname":"sunt","lname":"error","email":"nostrum","password":"et"}'
...
```bash
curl -X POST \
    "http://localhost/api/v1/polls/qui/choices" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -H "Authorization: Bearer {token}" \
    -d '{"message":"quia"}'
...
```bash
curl -X GET \
    -G "http://localhost/api/v1/polls/qui/choices" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -H "Authorization: Bearer {token}"
```
