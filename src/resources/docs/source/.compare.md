...
curl -X POST \
    ...
    -H "Accept: application/json" \
    -H "Authorization: Bearer {token}" \
    -d '{"fname":"quis","lname":"id","email":"atque","password":"atque"}'
...
```bash
curl -X GET \
    -G "http://localhost/api/v1/polls/at/choices" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -H "Authorization: Bearer {token}"
```
...
```javascript
const url = new URL(
    "http://localhost/api/v1/polls/at/choices"
);
...
```bash
curl -X POST \
    "http://localhost/api/v1/polls/quia/choices" \
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -H "Authorization: Bearer {token}" \
    -d '{"message":"animi"}'
